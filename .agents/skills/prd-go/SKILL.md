...
- The review/repair loop may run at most 3 rounds. If round 3 still fails, stop, publish a Chinese blocker summary on the PR, and ask the human for direction.
...
5. Spawn an Implementation subagent to create/switch a `codex/` branch, implement the Goal Issue, run sandboxed unit tests, run real local-host end-to-end verification, collect evidence, commit, push, and open a PR.
...
7. If review passes, spawn a Closure subagent to post final Chinese evidence and the current PR/check status. If review fails, spawn a Repair subagent and repeat review through at most 3 rounds.
